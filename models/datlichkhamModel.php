...
class datlichkhamModel
{
    ...
    //Cập nhật id, tình trạng tbldatlichkham
    public function UpdateConfirmDone($id)
    {
        $conn = connection::_open();
        $sql = "UPDATE tbldatlichkham 
                SET tinhTrang = '0' 
                WHERE id='{$id}'";
    ...
    //Lấy thông tin lịch khám bởi bác sĩ, ngày
    public function GetAllLichKhamByIdbacsiAndNgay($idbacsi, $ngay)
    {
        ...
        $sql = "SELECT * 
                FROM    tbldatlichkham A , 
                        tblbenhnhan B 
                WHERE A.idBenhnhan = B.id AND idBacsi = '{$idbacsi}'  AND ngayHen = '{$ngay}' AND tinhTrang = '1' ORDER BY soTT ASC ";
        $result = mysqli_query($conn, $sql)->fetch_all(MYSQLI_ASSOC);
        connection::_close($conn);
        return $result;
    }

    //Lưu thông tin lịch hẹn vào tbldatlichkham
    public function insert()
    {
        $conn = connection::_open();
        $sql = "INSERT INTO tbldatlichkham( idBenhnhan,
                                            idBacsi,
                                            soTT,
                                            ngayHen,
                                            gioHen,
    ...
    //Lưu thông tin lịch hẹn vao tbldatlichkham chu dong
    ...
    public function CheckDuplicateKhoa($id_benhnhan, $ngay, $idkhoa)
    {
        ...
        $sql = "SELECT * 
                FROM    tbldatlichkham A , 
                        tblbacsi  B 
                WHERE A.idBacsi = B.id AND A.idBenhnhan = {$id_benhnhan}  AND ngayHen='{$ngay}' AND B.idkhoa = {$idkhoa}  ";
    ...
    public function CheckDuplicateDatetime($id_benhnhan, $ngay, $indextime)
    {
        ...
        $sql = "SELECT * 
                FROM tbldatlichkham 
                WHERE idBenhnhan ='{$id_benhnhan}' AND ngayHen='{$ngay}' AND soTT='{$indextime}'  ";
        $data = mysqli_query($conn, $sql)->num_rows;
        connection::_close($conn);
        return $data;
    }
    ...
    //Kiểm tra lịch hẹn có trùng ngày giờ với người khác hay không
    public function CheckMultiBacsiDuplicateLich($id_benhnhan, $ngayhen, $stt)
    {
        ...
        $sql = "SELECT * 
                FROM tbldatlichkham A 
                WHERE  A.idBenhnhan = '{$id_benhnhan}' AND A.ngayHen = '{$ngayhen}' AND A.soTT = '{$stt}'";
        $data = mysqli_query($conn, $sql)->num_rows;
        connection::_close($conn);
        return $data;
    }

     //Kiểm tra lịch hẹn của bác sĩ có trùng ngày giờ hay không?
    public function CheckBacsiHasLichHen($id_bacsi, $ngayhen, $stt)
    {
        ...
        $sql = "SELECT * 
                FROM tbldatlichkham A 
                WHERE  A.idBacsi = '{$id_bacsi}' AND A.ngayHen = '{$ngayhen}' AND A.soTT = '{$stt}'";
        $data = mysqli_query($conn, $sql)->num_rows;
        connection::_close($conn);
        return $data;
    }

    //Kiểm tra lịch hẹn có trùng ngày giờ và bác sĩ, bệnh nhân với người khác hay không
    public function Check_001($id_bacsi, $id_benhnhan, $ngayhen)
    {
        ...
        $sql = "SELECT * 
                FROM tblbacsi 
                WHERE id='{$id_bacsi}' AND idKhoa 
                    in (SELECT B.idKhoa 
                        FROM tbldatlichkham A,tblbacsi B 
                        WHERE A.idBacsi = B.id AND A.idBenhnhan = '{$id_benhnhan}' AND A.ngayHen = '{$ngayhen}') ";
        $data = mysqli_query($conn, $sql)->num_rows;
        return $data;
    }
